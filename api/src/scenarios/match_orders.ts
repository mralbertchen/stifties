...
export async function scenario(): Promise<void> {
  ...
  const [
    ...
    rightMaker,
    matcherAccount
  ] = await zeroEx.getAvailableAddressesAsync();
  printData("Accounts", [
  ...
  const takerAssetData = ZeroEx.encodeERC20AssetData(
  ...
  txReceipt = await awaitTransactionMinedSpinnerAsync(
    "Matcher ZRX Approval",
    matcherZRXApprovalTxHash,
    zeroEx
  );
  ...
  const leftECSignature = await zeroEx.ecSignOrderHashAsync(
    leftOrderHashHex,
    leftMaker,
    {
  ...
  const rightECSignature = await zeroEx.ecSignOrderHashAsync(
    rightOrderHashHex,
    rightMaker,
    {
      prefixType: MessagePrefixType.EthSign,
  ...
  await fetchAndPrintBalancesAsync({ leftMaker, rightMaker, matcherAccount }, [
    zrxTokenContract,
    etherTokenContract
  ]);
  // Match the orders via 0x Exchange
  txHash = await zeroEx.exchange.matchOrdersAsync(
    leftSignedOrder,
    rightSignedOrder,
    matcherAccount,
    {
