...
export class WETH9Contract extends BaseContract {
    ...
    public withdraw = {
        async sendTransactionAsync(
            ...
            const encodedData = self._lookupEthersInterface('withdraw(uint256)').functions.withdraw(
                wad
            ).data;
            const txDataWithDefaults = await BaseContract._applyDefaultsToTxDataAsync(
                {
        ...
        getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = self._lookupEthersInterface('withdraw(uint256)').functions.withdraw(
                wad
            ).data;
            return abiEncodedTransactionData;
        },
    ...
    public deposit = {
        async sendTransactionAsync(
            ...
            [] = BaseContract._formatABIDataItemList(inputAbi, [], BaseContract._bigNumberToString.bind(self));
            const encodedData = self._lookupEthersInterface('deposit()').functions.deposit(
            ).data;
            const txDataWithDefaults = await BaseContract._applyDefaultsToTxDataAsync(
                {
                    to: self.address,
        ...
        getABIEncodedTransactionData(
            ...
            const inputAbi = self._lookupAbi('deposit()').inputs;
            [] = BaseContract._formatABIDataItemList(inputAbi, [], BaseContract._bigNumberToString);
            const abiEncodedTransactionData = self._lookupEthersInterface('deposit()').functions.deposit(
            ).data;
            return abiEncodedTransactionData;
        },
    };
